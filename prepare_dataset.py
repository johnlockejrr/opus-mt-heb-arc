#!/usr/bin/env python3
"""
Dataset preparation script for Hebrew-Aramaic translation model.
This script processes the aligned_corpus.tsv file and prepares it for training.
"""
...
def filter_valid_pairs(hebrew_text, aramaic_text):
    """
    Filter out invalid translation pairs.
    """
    # Both texts must be non-empty and cleaned
    hebrew_clean = clean_text(hebrew_text)
    aramaic_clean = clean_text(aramaic_text)
    
    if hebrew_clean is None or aramaic_clean is None:
        return False, None, None
    
    # Filter out very short or very long texts
    if len(hebrew_clean) < 5 or len(aramaic_clean) < 5:
        return False, None, None
    
    if len(hebrew_clean) > 1000 or len(aramaic_clean) > 1000:
        return False, None, None
    
    # Filter out texts that are too different in length (likely not good translations)
    length_ratio = len(hebrew_clean) / len(aramaic_clean)
    if length_ratio < 0.3 or length_ratio > 3.0:
        return False, None, None
    
    return True, hebrew_clean, aramaic_clean
...
def handle_duplicates(df, remove_duplicates=True):
    ...
    Args:
        ...
        remove_duplicates: If True, remove duplicates. If False, keep them but log info.
    ...
    hebrew_duplicates = df['Samaritan'].duplicated().sum()
    aramaic_duplicates = df['Targum'].duplicated().sum()
    ...
    logger.info(f"  - Hebrew text duplicates: {hebrew_duplicates}")
    ...
    if remove_duplicates:
        ...
        # (e.g., repeated phrases in biblical texts are common and valid)
...
def load_and_prepare_data(file_path, test_size=0.1, val_size=0.1, random_state=42, remove_duplicates=True):
    ...
    if 'Samaritan' not in df.columns or 'Targum' not in df.columns:
        raise ValueError("Expected columns 'Samaritan' and 'Targum' not found in the dataset")
    ...
    for idx, row in df.iterrows():
        ...
        is_valid, hebrew_clean, aramaic_clean = filter_valid_pairs(hebrew_text, aramaic_text)
...
def main():
    parser = argparse.ArgumentParser(description='Prepare Hebrew-Aramaic translation dataset')
    parser.add_argument('--input_file', default='aligned_corpus.tsv', 
                       help='Input TSV file path')
    parser.add_argument('--output_dir', default='./hebrew_aramaic_dataset',
    ...
    try:
        ...
        for i in range(min(3, len(dataset_dict['train']))):
            ...
            logger.info(f"  Hebrew: {example['hebrew'][:100]}...")
